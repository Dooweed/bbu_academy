...
def payme_payment_view(request):
    if not request.session["allow_media"] or not PurchaseRecord.objects.filter(id=request.session["allow_media"]).exists():  # Redirect to index if session has no record
