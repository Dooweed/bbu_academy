...
from payme_billing.utils import get_model_instance
...
def _CheckPerformTransaction(params):
    ...
    purchase = get_model_instance(purchase_id)
    if purchase is None or len(purchase) == 0:
...
def _CreateTransaction(params):
    ...
    # If transaction does not exist yet
        ...
        if result.is_error():
            ...
            if get_model_instance(purchase_id).update(state=1) != 1:
                return Error(RECEIPT_NOT_FOUND_ERROR)
    ...
    if transaction.exists():
        ...
        if transaction.state == 1:
            ...
            # Transaction is NOT timed out
                ...
                purchase = get_model_instance(transaction.record_id)
                if purchase.get().is_paid:
                    return Error(RECEIPT_PAID_ERROR)
                if purchase.update(state=4) != 1:
                    return Error(RECEIPT_NOT_FOUND_ERROR)
                purchase.get().complete_payment()
